  ...
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul
          class="cart-list"
          v-for="(cartInfo, index) in cartInfoValue"
          :key="cartInfo.id"
        >
          <li class="cart-list-con1">
            <input
              ...
              @click="updateById(cartInfo, $event)"
            />
          </li>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          class="chooseAll"
          type="checkbox"
          :checked="isAllChecked && cartInfoValue.length>0"
...
export default {
  ...
  computed: {
    ...
    isAllChecked() {
      return this.cartInfoValue.every((item) => item.isChecked == 1);
    },
    totalPrice() {
      let sum = 0;
      this.cartInfoValue.forEach((item) => {
  ...
  methods: {
    ...
    async updateById(cartInfo, event) {
      try {
        //如果是1则带给服务器0，如果是0则带给服务器1
    ...
    async deleteCheckedAll() {
      try {
        await this.$store.dispatch("deleteCheckedAll");
        this.getData();
      } catch (error) {
        error.message;
      }
